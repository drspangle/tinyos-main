...
implementation
{
	...
	command void PolicyManagerI.LoadPolicy(OblPolicy_t* policy)
		///< TODO: maybe I want to transform this into a task  
	{
		...
		//dbg("core", "PolicyRepositoryP:Load Policy policy[%d] -- pid: %d, evt: %d, enabled: %d\n", 0, policies[0].policy.policyId, policies[0].policy.eventId, policies[0].enabled); //nq
	...
	command void PolicyManagerI.RemovePolicy(policy_t pid)
		///< TODO: maybe I want to transform this into a task  
	{
		OblPolicy_t* policy;
		int i=0; //nq
